...
class MainController extends GetxController {
  ...
  late TextEditingController name, type, price, desc, number, gender, location;
  late dynamic color;
  ...
  addProductType() async {
    if (formKey.currentState!.validate()) {
      ...
      try {
        showdilog();
        var re = <String, dynamic>{"type": name.text};
      ...
      Get.back();
      showbar(
          "Product Added", "Product Type Added", 'please select image', false);
    }
  }
      ...
      actions: [
        CustomTextButton(
          ontap: () async {
            showdilog();
            try {
              await collectionReference3.doc(id).delete();
              update();
              Get.back();
              Get.back();
              showbar('delete', 'delete', 'Delivery deleted', true);
  ...
  editProduct(String id, int oPrice) async {
    price.text = oPrice.toString();
    Get.defaultDialog(
        content: SizedBox(
      height: 200,
      child: ListView(
  ...
  editDelivery(String id, String field) async {
    Get.defaultDialog(
        content: SizedBox(
      height: 200,
      child: ListView(
        children: [
          ...
          CustomTextButton(
              ...
              ontap: () async {
                try {
                  showdilog();
                  await collectionReference2.doc(id).update({field: name.text});
                  name.clear();
                  update();
                  Get.back();
                  Get.back();
  ...
  editType(String id, String ty) async {
    name.text = ty;
    Get.defaultDialog(
        content: SizedBox(
      height: 200,
